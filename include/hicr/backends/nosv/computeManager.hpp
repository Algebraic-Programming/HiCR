...
#include <hicr/backends/nosv/processingUnit.hpp>
...
class ComputeManager : public HiCR::ComputeManager
{
  ...
  {
    return std::make_unique<HiCR::backend::nosv::ProcessingUnit>(computeResource);
  }
  ...
  {
    return std::make_unique<HiCR::backend::nosv::ExecutionState>(executionUnit, argument);
  }
   ...
   */
  [[nodiscard]] __INLINE__ nosv::ProcessingUnit *getPUPointer(std::unique_ptr<HiCR::ProcessingUnit> &processingUnit)
  {
