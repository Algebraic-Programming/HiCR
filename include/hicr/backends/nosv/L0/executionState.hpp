...
class ExecutionState final : public HiCR::L0::ExecutionState
{
    ...
    : HiCR::L0::ExecutionState(executionUnit)
  {
    ...
    check(nosv_create(&_executionStateTask, _executionUnitTaskType, sizeof(executionStateMetaData_t), NOSV_CREATE_NONE));

    // Access the execution state task metadata
    auto metadata = (executionStateMetaData_t *)getTaskMetadata(_executionStateTask);
  ...
  void suspendImpl() override
  {
    ...
    auto metadata = (executionStateMetaData_t *)getTaskMetadata(self_task);
