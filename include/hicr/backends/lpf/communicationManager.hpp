...
class CommunicationManager final : public HiCR::CommunicationManager
{
  ...
  __INLINE__ uint8_t *serializeGlobalMemorySlot(const std::shared_ptr<HiCR::GlobalMemorySlot> &globalSlot) const override
  {
    char  *serialized;
    size_t size    = 0;
    auto   lpfSlot = dynamic_pointer_cast<lpf::GlobalMemorySlot>(globalSlot);
  ...
  __INLINE__ std::shared_ptr<HiCR::GlobalMemorySlot> deserializeGlobalMemorySlot(uint8_t *buffer, GlobalMemorySlot::tag_t tag) override
  {
    ...
    return std::make_shared<HiCR::backend::lpf::GlobalMemorySlot>(rank, slot, LPF_INVALID_MEMSLOT, tag);
  }
                                                                                ...
                                                                                HiCR::GlobalMemorySlot::tag_t                 tag) override
  {
    ...
    return std::make_shared<HiCR::backend::lpf::GlobalMemorySlot>(_rank, promotedSlot, lpfSwapSlot, tag, 0 /* global key */, memorySlot);
  }
  ...
  __INLINE__ void destroyPromotedGlobalMemorySlot(const std::shared_ptr<HiCR::GlobalMemorySlot> &memorySlot) override
  {
    ...
    auto lpfSlot = dynamic_pointer_cast<lpf::GlobalMemorySlot>(memorySlot);
  ...
  __INLINE__ void exchangeGlobalMemorySlotsImpl(HiCR::GlobalMemorySlot::tag_t tag, const std::vector<globalKeyMemorySlotPair_t> &memorySlots) override
  {
    ...
    auto localSlotKeys       = std::vector<HiCR::GlobalMemorySlot::globalKey_t>(localSlotCount);
    ...
    auto globalSlotKeys      = std::vector<HiCR::GlobalMemorySlot::globalKey_t>(globalSlotCount);
    ...
    for (size_t i = 0; i < globalSlotCount; i++)
    {
      ...
      if (globalSlotProcessId[i] == _rank)
      {
        ...
        localSlot             = (dynamic_pointer_cast<lpf::LocalMemorySlot>(memorySlot));
      ...
      auto memorySlot = std::make_shared<lpf::GlobalMemorySlot>(globalSlotProcessId[i], newSlot, swapValueSlot, tag, globalSlotKeys[i], globalSourceSlot);
  ...
  __INLINE__ void destroyGlobalMemorySlotsCollectiveImpl(HiCR::GlobalMemorySlot::tag_t tag)
  {
    ...
    {
      const auto memorySlot = std::dynamic_pointer_cast<HiCR::backend::lpf::GlobalMemorySlot>(slot);
  ...
  {
    // Getting up-casted pointer for the global memory slot
    auto memorySlot = dynamic_pointer_cast<lpf::GlobalMemorySlot>(memorySlotPtr);
                             ...
                             size_t                                             size) override
  {
    ...
    // Getting up-casted pointer
    auto dest = dynamic_pointer_cast<lpf::LocalMemorySlot>(destination);
                             ...
                             size_t                                             size) override
  {
    ...
    // Getting up-casted pointer
    auto dest = dynamic_pointer_cast<lpf::GlobalMemorySlot>(destination);
   ...
   */
  __INLINE__ globalKeyToMemorySlotMap_t getGlobalMemorySlots(GlobalMemorySlot::tag_t tag)
  {
   ...
   */
  __INLINE__ void fenceImpl(const std::shared_ptr<HiCR::LocalMemorySlot> &slot, size_t expectedSent, size_t expectedRcvd) override
  {
    auto          memSlot = dynamic_pointer_cast<lpf::LocalMemorySlot>(slot);
   ...
   */
  __INLINE__ void updateMessagesRecv(const std::shared_ptr<HiCR::LocalMemorySlot> &memorySlot)
  {
    size_t        msg_cnt = 0;
    auto          memSlot = dynamic_pointer_cast<lpf::LocalMemorySlot>(memorySlot);
    lpf_memslot_t lpfSlot = memSlot->getLPFSlot();
   ...
   * \param[out] memorySlot HiCr memory slot to update
   */
  __INLINE__ void updateMessagesSent(const std::shared_ptr<HiCR::LocalMemorySlot> &memorySlot)
  {
    size_t        msg_cnt = 0;
    auto          memSlot = dynamic_pointer_cast<lpf::LocalMemorySlot>(memorySlot);
    lpf_memslot_t lpfSlot = memSlot->getLPFSlot();
  ...
  __INLINE__ void queryMemorySlotUpdatesImpl(std::shared_ptr<HiCR::LocalMemorySlot> memorySlot) override { fenceImplSingle(memorySlot); }
  ...
  {
    auto          hicrSlot    = dynamic_pointer_cast<lpf::GlobalMemorySlot>(memorySlot);
   ...
   */
  __INLINE__ void fenceImplSingle(const std::shared_ptr<HiCR::LocalMemorySlot> &hicrSlot)
  {
    auto          memorySlot = dynamic_pointer_cast<lpf::LocalMemorySlot>(hicrSlot);
