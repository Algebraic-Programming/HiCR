...
class Task
{
   ...
   */
  __USED__ Task(L0::ExecutionUnit *executionUnit, taskEventMap_t *eventMap = NULL) : _executionUnit(executionUnit), _eventMap(eventMap)
  {
   ...
   */
  __USED__ inline const L0::ExecutionState::state_t getState()
  {
    // If the execution state has not been initialized then return the value expliclitly
    if (_executionState == NULL) return L0::ExecutionState::state_t::uninitialized;
   ...
   */
  __USED__ inline L0::ExecutionUnit *getExecutionUnit() const { return _executionUnit; }
   ...
   */
  __USED__ inline void initialize(std::unique_ptr<L0::ExecutionState> executionState)
  {
    if (getState() != L0::ExecutionState::state_t::uninitialized) HICR_THROW_LOGIC("Attempting to initialize a task that has already been initialized (State: %d).\n", getState());
  ...
  {
    if (getState() != L0::ExecutionState::state_t::initialized && getState() != L0::ExecutionState::state_t::suspended) HICR_THROW_RUNTIME("Attempting to run a task that is not in a initialized or suspended state (State: %d).\n", getState());
    ...
    if (getState() == L0::ExecutionState::state_t::suspended)
    ...
    if (getState() == L0::ExecutionState::state_t::finished)
  ...
  {
    if (getState() != L0::ExecutionState::state_t::running) HICR_THROW_RUNTIME("Attempting to yield a task that is not in a running state (State: %d).\n", getState());
  ...
  L0::ExecutionUnit *const _executionUnit;
   ...
   */
  std::unique_ptr<L0::ExecutionState> _executionState = NULL;
};
